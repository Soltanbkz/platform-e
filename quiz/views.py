...
class MCQuestionCreate(CreateView):
    ...
    def get_formset(self):
        return MCQuestionFormSet(self.request.POST, self.request.FILES)
...
class QuizTake(FormView):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if quizQuestions <= 0:
            messages.warning(request, f"Набор вопросов в тесте пуст. попробуйте позже!")
        ...
        if self.sitting is False:
            ...
            messages.info(
                request,
                f"Вы уже сдавали этот экзамен, и разрешается только одно занятие",
            )
            return redirect("quiz_index", self.course.slug)
